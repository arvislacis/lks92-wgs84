...
class LKS92WGS84:
	...
	def __init__(self):
		self.PI = math.pi							# Skaitlis pi
		self.A_AXIS = 6378137						# Elipses modeļa lielā ass (a)
		self.B_AXIS = 6356752.31414					# Elipses modeļa mazā ass (b)
		self.CENTRAL_MERIDIAN = 24 / 180 * math.pi	# Centrālais meridiāns
		self.OFFSET_X = 500000						# Koordinātu nobīde horizontālās (x) ass virzienā
		self.OFFSET_Y = -6000000					# Koordinātu nobīde vertikālās (y) ass virzienā
		self.SCALE = 0.9996							# Kartes mērogojuma faktors (reizinātājs)

	# Aprēķina loka garumu no ekvatora līdz dotā punkta ģeogrāfiskajam platumam
	def getArcLengthOfMeridian(self,phi):
		n = (self.A_AXIS - self.B_AXIS) / (self.A_AXIS + self.B_AXIS)
		alpha = ((self.A_AXIS + self.B_AXIS) / 2) * (1 + (math.pow(n, 2) / 4) + (math.pow(n, 4) / 64))
	...
	def getFootpointLatitude(self,y):
		n = (self.A_AXIS - self.B_AXIS) / (self.A_AXIS + self.B_AXIS)
		alpha = ((self.A_AXIS + self.B_AXIS) / 2) * (1 + (math.pow(n, 2) / 4) + (math.pow(n, 4) / 64))
		y = y / alpha
	...
	def convertMapLatLngToXY(self, phi, lambda1, lambda0):
		xy = [0, 0]

		ep2 = (math.pow(self.A_AXIS, 2) - math.pow(self.B_AXIS, 2)) / math.pow(self.B_AXIS, 2)
		nu2 = ep2 * math.pow(math.cos(phi), 2)
		N = math.pow(self.A_AXIS, 2) / (self.B_AXIS * math.sqrt(1 + nu2))
		t = math.tan(phi)
		t2 = t * t
		...
		# y koordināta
		xy[1] = self.getArcLengthOfMeridian(phi) + (t / 2 * N * math.pow(math.cos(phi), 2) * math.pow(l, 2)) + (t / 24 * N * math.pow(math.cos(phi), 4) * l4coef * math.pow(l, 4)) + (t / 720 * N * math.pow(math.cos(phi), 6) * l6coef * math.pow(l, 6)) + (t / 40320 * N * math.pow(math.cos(phi), 8) * l8coef * math.pow(l, 8))
		return xy

	# Pārveido punkta x, y koordinātas par ģeogrāfiskā platuma, garuma koordinātām (bez pārvietojuma un mērogojuma)
	def convertMapXYToLatLon(self, x, y, lambda0):
		latLng = [0, 0]
		...
		Nf = math.pow(self.A_AXIS, 2) / (self.B_AXIS * math.sqrt(1 + nuf2))
		Nfpow = Nf
	...
	def convertLatLonToXY(self, coordinates):
		lat = coordinates[0] * self.PI / 180
		lng = coordinates[1] * self.PI / 180
		xy = self.convertMapLatLngToXY(lat, lng, self.CENTRAL_MERIDIAN)
	...
	def convertXYToLatLon(self, coordinates):
		x = (coordinates[0] - self.OFFSET_X) / self.SCALE
		y = (coordinates[1] - self.OFFSET_Y) / self.SCALE
		latLng = self.convertMapXYToLatLon(x, y, self.CENTRAL_MERIDIAN)

		latLng[0] = latLng[0] / self.PI * 180
		latLng[1] = latLng[1] / self.PI * 180
